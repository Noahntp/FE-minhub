    ...
    if (isApiMode()) {
      try {
        ...
        const summary = response?.summary || {
          ...
          active_categories: items.filter((c: any) => c.status === "active").length,
          inactive_categories: items.filter((c: any) => c.status === "inactive").length,
          root_categories: items.filter((c: any) => c.parent_id === null).length,
          empty_categories: items.filter((c: any) => (c.course_count || 0) === 0).length,
        };
